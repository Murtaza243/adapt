...
class AdaPT_Linear(nn.Module):
    def __init__(self, size_in, size_out, bias=True, axx_mult='mul8s_acc'):
        ...
        self.axx_linear_kernel = load(name='PyInit_linear_'+axx_mult, sources=["/workspace/adapt/adapt/cpu-kernels/axx_linear.cpp"], extra_cflags = ['-DAXX_MULT=' + axx_mult + ' -fopenmp -O3' ], extra_ldflags=['-lgomp'], verbose=True)
...
class AdaPT_Conv2d(_ConvNd):
    ...
    dtype=None   
        ...
        #Jit compilation method for cpp extention
        #set PyInit_ prefix to comply with the python module name
        self.axx_conv2d_kernel = load(name='PyInit_conv2d_'+axx_mult, sources=["/workspace/adapt/adapt/cpu-kernels/axx_conv2d.cpp"], extra_cflags = ['-DAXX_MULT=' + axx_mult + ' -fopenmp -O3' ], extra_ldflags=['-lgomp'], verbose=True)
